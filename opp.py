...
st.markdown("""
    ...
    .shift-card { padding: 8px; border-radius: 6px; border-right: 8px solid #ccc; margin-bottom: 2px; }
...
def show_selection_dialog(shift_key, date_str, station, shift_name, v_type, req_df, history_scores, atan_col, hours_col):
    ...
    if avail_df.empty:
        ...
        for idx, row in avail_df.iterrows():
            label = f"{row['שם']} | מאזן: {int(row['balance'])} | {row['תחנה']} | {row[hours_col]}"
            options.append((label, row['שם']))

        selected_label = st.radio("בחר עובד:", [opt[0] for opt in options], index=None)

        # תיקון כאן: width="stretch"
        if st.button("אשר שיבוץ", type="primary", width="stretch"):
            if selected_label:
    ...
    for date in dates:
        for idx, s_row in shifts_template.iterrows():
            ...
            if shift_key in st.session_state.cancelled_shifts:
                ...
                status, assigned_name = "חסר", "⚠️ לא שובץ"
            else:
            ...
            summary_data.append({
                "תאריך": date, "יום": get_day_name(date), "משמרת": s_row['משמרת'],
                "תחנה (בפועל)": s_row['תחנה'], "שם העובד": assigned_name,
                "תחנה (מקורית)": req_station, "שעות (מקוריות)": req_hours, "סטטוס": status
            })
            
    if summary_data:
        # תיקון כאן: width="stretch"
        st.dataframe(
            pd.DataFrame(summary_data).style.apply(highlight_table_rows, axis=1), 
            width="stretch", hide_index=True
        )

    st.divider()
    # תיקון כאן: width="stretch"
    if st.button("💾 שמירה סופית ועדכון היסטוריה", type="primary", width="stretch"):
        count = update_db_balance(st.session_state.final_schedule)
        st.balloons(); st.success(f"נשמר בהצלחה! {count} שיבוצים עודכנו."); st.session_state.final_schedule = {}
else:
